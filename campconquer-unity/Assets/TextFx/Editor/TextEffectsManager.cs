...
namespace TextFx
{
	public class TextEffectsManager : EditorWindow
	{
		...
		TEXTFX_IMPLEMENTATION m_textfx_implementation;
...
//			m_save_button_texture = AssetDatabase.LoadAssetAtPath(m_textFxRootDirectoryPath + "Editor/Icons/SaveButton.png", typeof(Texture2D)) as Texture2D;
		...
		[MenuItem("Tools/TextFx Internal/Update Animation Names List", false)]
		public static void UpdateAnimSectionNameFile()
		{
			tfxJSONObject animNamesData = new tfxJSONObject();

			string[] animSectionFolderNames = new string[] { "Intros", "Mains", "Outros" };

			foreach(string folderName in animSectionFolderNames)
			{
				tfxJSONArray animNames = new tfxJSONArray();

				DirectoryInfo introAnimsDirectory = new DirectoryInfo(m_textFxRootDirectoryPath + "Resources/" + folderName);

				foreach(FileInfo animFile in introAnimsDirectory.GetFiles())
				{
					if(!animFile.Name.Contains(".meta"))
						animNames.Add(new tfxJSONValue(animFile.Name.Replace(".json", "")));
				}

				animNamesData[folderName] = animNames;
			}


			StreamWriter sw = new StreamWriter(m_textFxRootDirectoryPath + "Resources/textfxAnimNames.txt");
            sw.WriteLine(animNamesData.ToString());
			sw.Flush();
			sw.Close();
	    ...
	    void OnGUI ()
		{
			...
			if(Selection.gameObjects.Length == 1  && ((m_textfx_animation_interface == null || m_textfx_animation_interface.Equals(null)) || !Selection.gameObjects[0].Equals(m_textfx_animation_interface.GameObject)))
			{
				...
				if(textfx_animation_interface != null)
				{
					...
					m_textfx_implementation = m_textfx_animation_interface.TextFxImplementation;
...
//			}
			...
			if(Application.unityVersion.Contains("5.2.1p") && m_textfx_implementation == TEXTFX_IMPLEMENTATION.UGUI && !TextFxUGUI.USING_UGUI_API_PATCH_FIX)
			{
				// Using the latest patch release for 5.2.1, with unity UI API changes
				bool currentRichTextState = GUI.skin.box.richText;
                GUI.skin.box.richText = true;

                GUI.Box(
					new Rect(0, gui_y_offset, Instance.position.width, LINE_HEIGHT *4),
					new GUIContent("<color=red>You are using a <b>5.2.1 patch</b> editor release.\nThese patch releases have breaking changes in the Unity UI api, causing the UGUI TextFx component to no longer work.\n<b>TO FIX</b>: Please uncomment the <b>UNITY_5_2_1p2_FIX</b> pragma define on line 3 of your TextFxUGUI.cs script.</color>"));
